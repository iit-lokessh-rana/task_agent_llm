...
def install_uv_and_run_datagen(user_email: str = None):
    ...
    if not user_email:
        user_email = "23f2005068@ds.study.iitm.ac.in"
...
from pathlib import Path
from llm_utils import chat_completion  # Importing chat_completion for LLM usage
...
def preprocess_image(image_path: str) -> Image:
    ...
    image = enhancer.enhance(2)  # You can adjust this factor for better results
    
    # Resize the image for better OCR results
    image = image.resize((image.width * 2, image.height * 2), Image.Resampling.LANCZOS)  # Increase size by a factor of 2
    
    return image

def extract_credit_card_number(input_image: str, output_file: str):
    """
    Extract the credit card number from an image using OCR and LLM, then save it to output_file.
    ...
    input_path = Path(input_image)  # Convert to Path object
    output_path = Path(output_file)  # Convert to Path object
    ...
    # Clean up the OCR result by removing non-digit characters and focusing on possible card numbers
    cleaned_text = "".join([char for char in extracted_text if char.isdigit()])
    
    # Extract the first 16 digits from the cleaned text
    card_number = cleaned_text[:16]  # Take the first 16 digits
    
    # If we don't get a valid card number, handle the case (e.g., prompt the LLM)
    if len(card_number) != 16:
        # Fallback: Ask the LLM to extract the card number
        ...
        card_number = "".join([char for char in card_number if char.isdigit()])[:16]  # Ensure 16 digits
...
# Example usage with the correct image path
extract_credit_card_number('/Users/mish/Documents/llm/data/credit-card.png', '/Users/mish/Documents/llm/data/credit-card.txt')
